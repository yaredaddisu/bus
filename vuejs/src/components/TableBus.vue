...
import busView from "@/views/Bus/busView.vue";
...
//const items = computed(() => mainStore.buses)
...
const loading = computed(() => store.state.buses.loading);

store.dispatch("getBuses", { search: search.value });
...
const numPages = computed(() => Math.ceil(buses.value.length / perPage.value));
...
function yesDeleteAll(delVal) {
  ...
  if (delVal.length) {
    store.dispatch("deleteAllBus", delVal).then(() => {
      store.commit("notify", {
        type: "success",
...
function select() {
  selected.value = [];
  if (!selectAll.value) {
    for (let i in buses.value) {
      selected.value.push(buses.value[i]);
    }
  }
}

function updateCheckall() {
  if (buses.value.length == selected.value.length) {
...
function busTicket(id) {
  router.push(`/bus-ticket/${id}`);
}
</script>
...
<template>
  ...
  <div>
        ...
        <div class="w-auto" id="navbar-cta">
          ...
          >
            ...
            <div>
              ...
              >
                Total
                <b class="border border-slate-500 p-1 rounded-full">{{ buses.length }}</b>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="overflow-x-auto w-full">
      <table>
        ...
        <tbody>
          <tr v-for="bus in itemsPaginated" :key="bus.id">
            <td>
              <input
                type="checkbox"
                :value="bus"
            ...
            <td data-label="Bus Number">
              <span @click="busTicket(bus.id)">Ticket</span>
              {{ bus.busNumber }}
            </td>
            ...
            <td data-label="Availability">
              <span
                ...
                >{{ bus.availability == 1 ? "Active" : "In Active" }}
              </span>
            </td>
            <td data-label="Created" class="lg:w-1 whitespace-nowrap">
              <small class="text-gray-500 dark:text-slate-400" :title="bus.updated_at">{{
                bus.updated_at
              }}</small>
            </td>
  ...
  <div v-if="!buses.length" class="w-full">
    <CardBox>
