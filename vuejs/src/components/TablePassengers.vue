...
import { mdiEye, mdiTrashCan, mdiOpenInNew, mdiCloudPrint } from "@mdi/js";
...
import ReserveTicket from "@/components/ReserveTicket.vue";
...
//const items = computed(() => mainStore.routes)

const search = ref("");
const sortField = ref("updated_at");
const sortDirection = ref("desc");

const routes = computed(() => store.state.passengers.data);
const loading = computed(() => store.state.passengers.loading);

store.dispatch("getPassengers", { id: route.params.id });

watch(search, (newValue, oldValue) => {
  store.dispatch("getPassengers", {
    search: search.value,
...
function getPassengers(url = null) {
  store.dispatch("getPassengers", {
    url,
    search: search.value,
    sort_field: sortField.value,
...
const numPages = computed(() => Math.ceil(routes.value.length / perPage.value));
...
function yesDeleteAll(delVal) {
  ...
  if (delVal.length) {
    store.dispatch("deleteAllRoute", delVal).then(() => {
      store.commit("notify", {
        type: "success",
...
function select() {
  selected.value = [];
  if (!selectAll.value) {
    for (let i in routes.value) {
      selected.value.push(routes.value[i]);
    }
  }
}

function updateCheckall() {
  if (routes.value.length == selected.value.length) {
...
let reservation = ref({});
function modelReservation(route) {
  isVisible.value = !isVisible.value;
  reservation.value = route;
}
...
function exportToExcel(data) {
      ...
      const worksheet = workbook.addWorksheet('Bus Route and Seat Data');

      // Add headers for the bus route and seat data
      worksheet.addRow([
        'Reference',
        'Bus Number',
        'Source City',
      ...
      data.forEach((reservation) => {
        const { bus, route, seat, reference, firstName, lastName, gender, contactNumber, bookingDate, journeyDate, status } = reservation;

        // Add row for bus route information
        worksheet.addRow([
          reference,
          bus.busNumber,
          route.source_city,
          ...
          route.distance,
          route.price,
          seat.map(s => s.seatNumber).join(', '), // Combine seat numbers with comma
          route.price * seat.length, // Calculate total price
          firstName,
          lastName,
          gender,
          contactNumber,
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        ...
        saveAs(blob, 'bus_route_and_seat_data.xlsx');
      });
    }
...
<template>
  ...
  <CardBoxMember v-if="showModal" @openModal="openModal" title="Update Route " has-cancel>
    <RoutesView :id="data.id" />
  </CardBoxMember>
  <Notification />
      ...
      >
        ...
        <div class="w-auto" id="navbar-cta">
          ...
          >
            ...
            <div>
              <button @click="exportToExcel(selected)">Export to Excel</button>
      ...
      <table>
        <thead>
          ...
          >
            ...
            <th
              ...
              :sort-field="sortField"
              :sort-direction="sortDirection"
              @click="sortroutes('contactNumber')"
            >
              contactNumber
            </th>
        ...
        <tbody>
          <tr v-for="route in itemsPaginated" :key="route.id">
            ...
            <div class="fixed inset-0 overflow-y-auto" v-if="isVisible">
              <div class="flex items-center justify-center min-h-screen">
                ...
                >
                  ...
                  <div>
                    <ReserveTicket :reservation="reservation" />
                  </div>
            ...
            <td data-label="route">
              {{ route.route.source_city }} - {{ route.route.destination_city }}
            </td>
            <td data-label="route">
              {{ route.contactNumber }}
            </td>
            ...
            <td data-label="route">
              <div class="py-1 justify-center" v-for="seat in route.seat" :key="seat.id">
                <div>{{ seat.seatNumber }}</div>
              </div>
            </td>
            <td data-label="route">
              {{ $filters.currencyUSD(route.price) }}
            </td>
            <td data-label="route">
              {{ route.bookingDate }}
            </td>
            ...
            <td data-label="Status">
              <span
                ...
                >{{ route.status == "booked" ? "Confirmed" : "Not Confirmed" }}
              </span>
            </td>
            <td data-label="Created" class="lg:w-1 whitespace-nowrap">
              <small
                class="text-gray-500 dark:text-slate-400"
                :title="route.updated_at"
                >{{ route.updated_at }}</small
              >
            </td>
            <td class="before:hidden lg:w-1 whitespace-nowrap">
              <BaseButtons type="justify-start lg:justify-end" no-wrap>
                <BaseButton
                  ...
                  :icon="mdiCloudPrint"
                  small
                  @click="modelReservation(route)"
                />
                <BaseButton
                  color="info"
                  :icon="mdiOpenInNew"
                  small
                  @click="openModal(route.id)"
                />
                <BaseButton
                  color="danger"
                  :icon="mdiTrashCan"
                  small
                  @click="deleteBus(route.id)"
                />
              </BaseButtons>
            </td>
          </tr>
