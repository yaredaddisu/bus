...
if (props.id) {
  store.dispatch("showBus", { id: props.id });
}

// instead, use a getter:

watch(
  () => store.state.currentBus.data,
  (newVal, oldVal) => {
    model.value = {
  ...
  <div>
    <div class=" ">
      ...
      <div class="p-4 md:p-5">
        <form class="space-y-4" @submit.prevent="addBus">
          <Notification2 />
          <Error />
          <div>
            <input
              v-model="model.busNumber"
              type="text"
              name="Bus Number"
              id="Bus Number"
          ...
          <div>
            <label class="m-2" for="">Availability</label>
            <input
              v-model="model.availability"
              type="checkbox"
              name="Availability"
              id="Availability"
              placeholder="Bus  Availability"
            />
          </div>
          ...
          >
            ...
            {{ model.id ? "Update Bus" : "Create Bus" }}
          </button>
        </form>
      </div>
    </div>
